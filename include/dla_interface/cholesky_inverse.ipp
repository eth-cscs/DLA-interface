...
void choleskyInverse(UpLo uplo, DistributedMatrix<ElType>& mat, SolverType solver, int print_timers) {
  ...
  switch (solver) {
    ...
    case ScaLAPACK: {
      ...
      int info = 0;
      {
        ...
        scalapack_wrappers::ppotri(uplo, mat_scalapack.size().first, std::get<0>(matrix_info),
      ...
      if (comm_grid.id2D() == std::make_pair(0, 0)) {
        ...
        timer_part.print_elapsed(timer_index[1], timer_index[2],
                                 "Cholesky Inverse (ScaLAPACK) time: ", flop);
    ...
    case DPlasma: {
      ...
      int info = 0;
      {
        ...
        info = dplasma_wrappers::dplasma_run<dplasma_wrappers::ppotri<ElType>>(
      ...
      if (comm_grid.id2D() == std::make_pair(0, 0)) {
        ...
        timer_part.print_elapsed(timer_index[1], timer_index[2],
                                 "Cholesky Inverse (DPlasma) time: ", flop);
        timer_part.print_elapsed(timer_index[2], timer_index[3], "Back conversion a: ");
      }
      if (info != 0) {
        if (info < 0)
    ...
    default:
      throw std::invalid_argument(
          errorMessage("Cholesky Inverse is not available for solver ", solver));
  }
  int index_end = timer_full.save_time();
  if (comm_grid.id2D() == std::make_pair(0, 0)) {
    timer_full.print_elapsed(0, index_end, "DLA Cholesky Inverse time: ", flop);
  }
