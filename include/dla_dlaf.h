	...
	namespace hpx_wrappers {
		...
		inline void start(int argc, char** argv, std::vector<std::string> cfg) {
			...
			p.rp_callback = [](auto& rp, auto) {
				...
				std::cout << " -> number of MPI tasks = " << ntasks << std::endl; 
				// HPX needs thread pool MPI
				//
			...
			// convert cxxopts cmd line arguments to hpx cmd line options
			std::string word = "--nr_threads"; 
			std::string nr_threads;
			for(int i=0; i<argc; i++) {
				//std::cout << " -> old argv[" << i << "] = " << argv[i] << std::endl;
				std::size_t found = std::string(argv[i]).find(word);
				if (found!=std::string::npos) {
					nr_threads = "--hpx:threads=" + std::string(argv[i+1]);
				//	std::cout << " -> number of HPX threads: " << nr_threads << std::endl;
				}
			}
		
			char *hpx_argv[] = {argv[0], (char*)(nr_threads.c_str())};
			hpx::start(nullptr, 2, hpx_argv, p);
			hpx::suspend();
		}
